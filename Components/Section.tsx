	...
	return (
		<section className="portfolio__detail">
			...
			{media && (<div key={media[0].id} className="portfolio__detail-thumb">
				<Image src={`${process.env.NEXT_PUBLIC__NODE_ENV === 'development' ? process.env.BACK_URL : process.env.NEXT_PUBLIC__NODE_BACK}${media[0].img.data.attributes.url}`} width={media[0].img.data.attributes.width} height={media[0].img.data.attributes.height} alt={media[0].alt} data-speed="auto" />
			</div>)}
			 ...
			 <div className="portfolio__detail-content">
			 	<div className="container g-0 line pt-140">
					...
					{(media && media.length >= 2) && (<div key={media[1]?.id} className="block-thumb">
						<Image
							src={`${process.env.NEXT_PUBLIC__NODE_ENV === 'development' ? process.env.BACK_URL : process.env.NEXT_PUBLIC__NODE_BACK}${media[1]?.img.data.attributes.url}`}
...
export const SectionTechStack = ({ title, description, link, Items, Uptitle }:SectionProps) => {
	...
	<Image src={`${process.env.NEXT_PUBLIC__NODE_FRONT}${item.Image.data[0].attributes.url}`}
