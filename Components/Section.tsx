...
export const SectionTechStack = ({ title, description, link, Items, Uptitle }) => {
	return (
...
export const SectionWhatWeAlsoDo = ({ title, description, link, Items, Uptitle }) => {
	return (
							...
							<div className="service__items-7 animation_service_7">
								{Items.map((item, index) =>
									<div className="service__item-7"
										...
										<ul>
											{item.List_item.map((item, index) => <li key={item.id}>+ {item.Title}</li>)}
										</ul>
									</div>)}
...
export const SectionHowWeWork = ({ title, description, link, Items, Uptitle }) => {
	const Card = ({ num, title, description }) => <div className="swiper-slide workflow__slide fade_left">
...
export const SectionProblems = ({ title, description, link, Items }) => {
	return (
...
export const SectionPitch = ({ title, description, link }) => {
	return (
...
export const SectionScreen = ({ title, description }) => {
...
const Section = ({ title, description, link, Items, Uptitle }) => {
	return (
									...
									{Items.map((item, index) =>
										<div key={item.id}
											...
											<div className="animation__service_page">
												...
												<div dangerouslySetInnerHTML={{ __html: item.Description }}/>
												<ul>
													{item.List_item.map(item => <li key={item.id}>+ {item.Title}</li>)}
												</ul>
